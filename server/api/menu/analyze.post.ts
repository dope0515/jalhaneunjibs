...
/** Qwen 3.6은 기본 thinking 모드 → JSON 모드와 충돌하므로 reasoning 비활성화 필요 */
...
>>>>>>> fe68c4d19848374d3b1d311c9a22684453dee1c1
  ...
  try {
    ...
    const groqError = error?.error ?? error?.body?.error
    ...
    if (status === 400 && groqError?.code === 'json_validate_failed') {
      throw createError({
        statusCode: 422,
        statusMessage: '메뉴판 분석 결과를 JSON으로 변환하지 못했습니다. 이미지를 더 선명하게 다시 시도해주세요.',
      })
    }

    throw createError({
      statusCode: status && status >= 400 && status < 600 ? status : 500,
      statusMessage: groqError?.message || error.message || '메뉴 분석 중 오류가 발생했습니다.',
    })
  }
})
